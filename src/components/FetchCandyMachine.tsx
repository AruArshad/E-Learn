  ...
  return (
    <div>
      <input
        ...
        placeholder="Enter Candy Machine v1 Address"
      ...
      {pageItems && (
        <div>
          <div className={styles.gridNFT}>
            {pageItems.map((nft) => (
              <div>
                <ul className="text-xl my-3">{nft.name}</ul>
              <img className="h-auto max-w-full rounded-lg mx-auto" src={nft.image}/>
              {nft.symbol == "" ? null :(
                <p className="text-sm my-2">Token Symbol: {nft.symbol}</p>
              )}
              {nft.description == "" ? null :(
               <p className="text-sm my-2 mx-auto">Desc: {nft.description}</p>
              )}
              </div>
            ))}
          </div>
          ...
          <button
            className="px-8 m-2 btn animate-pulse bg-gradient-to-r from-[#9945FF] to-[#14F195] hover:from-pink-500 hover:to-yellow-500 ..."
            onClick={pageItems.length < 1 ? null : next}
          >
            Next
          </button>
        </div>
